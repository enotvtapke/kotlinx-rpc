...
object FirRemoteMethodCallChecker : FirFunctionCallChecker(MppCheckerKind.Common) {
    context(context: CheckerContext, reporter: DiagnosticReporter)
    override fun check(expression: FirFunctionCall) {
        if (expression.dispatchReceiver == null) return
        val classSymbol = expression.dispatchReceiver!!.resolvedType.toClassSymbolVS(context.session)
